...
export function Navbar() {
  ...
  return (
    <>
      {/* Desktop Sidebar */}
      <aside className="hidden md:flex md:flex-col w-64 p-6 bg-white dark:bg-black border-r border-gray-300 dark:border-gray-800 text-gray-700 dark:text-gray-400 font-mono justify-center md:fixed md:inset-y-0 md:left-0 md:h-screen md:overflow-y-auto">
        <nav>
          <ul>
            {navItems.map((item) => {
                ...
                return (
                  <li key={item.name} className="mb-4">
                    <a
                      ...
                      className={`flex items-center p-2 rounded-md transition-all duration-300 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-gray-100 dark:hover:bg-gray-900`}
                    >
                ...
                <li key={item.name} className="mb-4">
                  <Link
                    ...
                    className={`flex items-center p-2 rounded-md transition-all duration-300 ${
                      isActive
                        ? 'text-blue-700 dark:text-blue-400 bg-gray-100 dark:bg-gray-800 scale-95 translate-x-2'
                        : 'hover:text-blue-600 dark:hover:text-blue-400 hover:bg-gray-100 dark:hover:bg-gray-900'
                    }`}
                  >
                    <FontAwesomeIcon icon={item.icon} className="w-5 h-5 mr-3" />
                    <span>{item.name}</span>
          ...
          {navItems
            ...
            .map((item) => {
              ...
              return (
                ...
                >
                  {isActive && <span className="absolute -top-1 h-1 w-8 bg-blue-600 dark:bg-blue-400 rounded-full"></span>}
